...
| http.cacheViews      | boolean | Enables or disables the view caching in production environment.                                                 | false            |
...
```php
public function <METHOD>(string $path, ...$options): Marvic\HTTP\Message\Response
```
...
```php
public function controller(string $classname, Callable $callback): Marvic\Routing\Router
```

#### Router::map()

```php
public function map(array $app): Marvic\Routing\Router
```
...
Avaliable Methods: GET, POST, PUT, DELETE, HEAD, PATCH, OPTIONS, TRACE, CONNECT, ANY.
...
Avaliable Methods: GET, POST, PUT, DELETE, HEAD, PATCH, OPTIONS, TRACE, CONNECT.
